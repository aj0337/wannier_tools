...
subroutine ek_bulk_line
   ...
   integer :: ik, il, ig, io, i, j, knv3, ierr, i1, i2
   ...
   open(unit=1012, file='Hamk.dat')
   do ik= 1+cpuid, knv3, num_cpu

      ! write(1012, '(3f13.6)') k(:)
      k = (/0.333330,0.333330,0.000000 /)
      write(1012, '(3f13.6)') k(:)
      ...
      if (index(KPorTB, 'KP')/=0)then
         ...
         if (index(Particle,'phonon')/=0.and.LOTO_correction) then
            ...
            do i1=1, Num_wann
               do i2=1, Num_wann
                  write(1012, '(2f13.6)') Hamk_bulk(i1,i2)
               enddo
            enddo
         endif
      endif
...
subroutine sparse_ekbulk
   ...
   k3 = (/3.333300000000000E-002,  3.333300000000000E-002,  0.000000000000000E+000 /)
   open(unit=1013, file='psik.dat')
   ...
   call now(time3)
   eigv(1:neval)= W(1:neval)

   do ib= 1, neval
      psi(:)= zeigv(:, ib)  !> the eigenvector of ib'th band
      write(1013, '(2f13.6)') k3
      write(1013, '(1i5, 1f13.6)') ib, eigv(ib, 0)
      write(1013, '(2f13.6)') psi(:)
   enddo
   close(1013)
   call EXIT()
   ...
   !    open(unit=1013, file='psik.dat')
   !    k3 = K3points(:, ik)
   !    K = (/3.333300000000000E-002,  3.333300000000000E-002,  0.000000000000000E+000 /)
   !    call now(time1)
   !    call ham_bulk_coo_sparsehr(k3,acoo,icoo,jcoo)
   !    acoo= acoo/eV2Hartree
   !    nnz= splen
   ...
   !       if(ik.eq.21)then
   !          write(1013, '(2f13.6)') k3
   !          write(1013, '(1i5, 1f13.6)') ib, eigv(ib, ik)
   !          write(1013, '(2f13.6)') psi(:)
   !       endif
   !       ! write(1013, '(1i5, 1f13.6)') ib, eigv(ib, ik)
   !       ! write(1013, '(2f13.6)') psi(:)
